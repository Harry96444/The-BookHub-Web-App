...
public class UserDAOImpl implements UserDAO {
	...
	public boolean createAccount(userDtls user) {
		boolean f=false;
		try {
			String sql="insert into usertab(name,email,phno,password,adress,landmark,city,state,zip) values(?,?,?,?,?,?,?,?,?)";
	...
	public userDtls loginUser(String email, String password) {
		...
		userDtls user=null;
		try {
			String sql="select * from usertab where email=? and password=?";
			PreparedStatement ps=conn.prepareStatement(sql);
	...
	public boolean addAdress(String ad, String la, String city, String st, String pin,int id) {
		boolean f=false;
		try {
			String sql="update usertab set adress=?,landmark=?,city=?,state=?,zip=? where id=?";
			PreparedStatement ps=conn.prepareStatement(sql);
			ps.setString(1,ad);
			ps.setString(2,la);
			ps.setString(3,city);
	...
	public boolean editProfile(userDtls us) {
		boolean f=false;
		try {
			String sql="update usertab set name=?,email=?,phno=? where id=? and password=?";
			PreparedStatement ps=conn.prepareStatement(sql);
